...
@rt
def index(req):
    status = req.query_params.get("filter", "all")  # "all", "active", or "completed"

    if status == "active":
        todos = con.execute("SELECT * FROM todos WHERE done = 0 ORDER BY id DESC").fetchall()
    elif status == "completed":
        todos = con.execute("SELECT * FROM todos WHERE done = 1 ORDER BY id DESC").fetchall()
    else:
        todos = all()
    ...
    return Container(
        Div(cls="text-center mb-8")(
            ...
            P("Stay organised and get things done", cls="text-muted-foreground")),

        # Filter buttons
        Div(cls="flex justify-center gap-4 mb-6")(
            Button("All", hx_get="/", hx_target="body", cls=ButtonT.ghost),
            Button("Active", hx_get="/?filter=active", hx_target="body", cls=ButtonT.ghost),
            Button("Completed", hx_get="/?filter=completed", hx_target="body", cls=ButtonT.ghost)
        ),
